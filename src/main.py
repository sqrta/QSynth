...
def teleportation(n, x, y):
    ...
    expr = SumExpr(
        BVtrunc(z, 2 * n - 1, n) == BVtrunc(phi, n - 1) ^ BVtrunc(z, 3 * n - 1, 2 * n),
        BVtrunc(z, n - 1),
    )
    Output = [c0, expr]
...
if __name__ == "__main__":
    ...
    prog = synthesis(
        spec, StandardGateSet, hypothesis=lambda n, x, y: And(BVtrunc(x, n) == 0)
    )
    end = time.time()
    ...
    prog = synthesis(
        spec,
        StandardGateSet,
        hypothesis=lambda n, x, y: And(BVtrunc(x, 2 * n) == 0, n > 0),
    )
    end = time.time()
    ...
    prog = synthesis(
        spec,
        StandardGateSet,
        hypothesis=lambda n, x, y: And(BVtrunc(x, 3 * n, n + 1) == 0, n > 1),
    )
    end = time.time()
    ...
    prog = synthesis(
        spec, StandardGateSet, hypothesis=lambda n, x, y: And(BVref(x, 0) == 0)
    )
    end = time.time()
    ...
    prog = synthesis(
        spec, caseSet + StandardGateSet, hypothesis=lambda n, x, y: BVref(x, 0) == 0
    )
    end = time.time()
    ...
    prog = synthesis(
        spec, StandardGateSet + caseSet, hypothesis=lambda n, x, y: BVref(x, 0) == 0
    )
    end = time.time()
    ...
    prog = synthesis(
        spec,
        StandardGateSet,
        hypothesis=lambda n, x, y: And(
            BVtrunc(x, 2 * n - 1, n + 1) == 0,
            n > 0,
            BVref(y, 2 * n - 1) == BVReductAnd(x, n - 1),
        ),
        base=2,
    )
    end = time.time()
    ...
    caseSet = [
        [Subtractor("sub", params={"c": c}), Cadder("cadd", params={"c": c}), X("x")]
    ]
    start = time.time()
    spec = PPSA(beta=lambda n: 1, phaseSum=inversionSpec)
    prog = synthesis(
        spec,
        caseSet + StandardGateSet,
        hypothesis=lambda n, x, y: And(
            n > 0, BVtrunc(x, 2 * c + n - 1, c) == (bv(1) << (c + n))
        ),
    )
    end = time.time()
